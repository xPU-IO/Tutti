...
Status validate_striped_local_nvme_backend(
    const ResourceSpec& resource, const BackendSpec& backend,
    ...
    if (backend.stripe_unit == 0 || backend.stripe_unit % 4096 != 0) {
        return invalid(
            "striped backend stripe_unit must be non-zero and 4096-byte aligned");
    }
    return Status::Ok();
}

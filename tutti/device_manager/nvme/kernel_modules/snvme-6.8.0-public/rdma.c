...
static int nvme_rdma_alloc_io_queues(struct nvme_rdma_ctrl *ctrl)
{
	...
	ret = snvme_set_queue_count(&ctrl->ctrl, &nr_io_queues);
	if (ret)
		return ret;
...
static int nvme_rdma_alloc_tag_set(struct nvme_ctrl *ctrl)
{
	...
	return snvme_alloc_io_tag_set(ctrl, &to_rdma_ctrl(ctrl)->tag_set,
			&nvme_rdma_mq_ops,
		...
		bool new)
{
	...
	if (new) {
		error = snvme_alloc_admin_tag_set(&ctrl->ctrl,
	...
	error = snvme_enable_ctrl(&ctrl->ctrl);
	if (error)
		goto out_stop_queue;
	...
	snvme_unquiesce_admin_queue(&ctrl->ctrl);

	error = snvme_init_ctrl_finish(&ctrl->ctrl, false);
	if (error)
...
out_quiesce_queue:
	snvme_quiesce_admin_queue(&ctrl->ctrl);
...
out_remove_admin_tag_set:
	if (new)
		snvme_remove_admin_tag_set(&ctrl->ctrl);
out_free_async_qe:
	if (ctrl->async_event_sqe.data) {
		nvme_rdma_free_qe(ctrl->device->dev, &ctrl->async_event_sqe,
			sizeof(struct nvme_command), DMA_TO_DEVICE);
...
static int nvme_rdma_configure_io_queues(struct nvme_rdma_ctrl *ctrl, bool new)
{
	...
	if (!new) {
		snvme_start_freeze(&ctrl->ctrl);
		snvme_unquiesce_io_queues(&ctrl->ctrl);
		if (!snvme_wait_freeze_timeout(&ctrl->ctrl, NVME_IO_TIMEOUT)) {
			/*
...
out_wait_freeze_timed_out:
	snvme_quiesce_io_queues(&ctrl->ctrl);
	nvme_sync_io_queues(&ctrl->ctrl);
	nvme_rdma_stop_io_queues(ctrl);
out_cleanup_tagset:
	nvme_cancel_tagset(&ctrl->ctrl);
	if (new)
		snvme_remove_io_tag_set(&ctrl->ctrl);
out_free_io_queues:
...
static void nvme_rdma_reconnect_or_remove(struct nvme_rdma_ctrl *ctrl)
{
	...
	if (nvmf_should_reconnect(&ctrl->ctrl)) {
		...
		queue_delayed_work(s_nvme_wq, &ctrl->reconnect_work,
...
static int nvme_rdma_setup_ctrl(struct nvme_rdma_ctrl *ctrl, bool new)
{
	...
	changed = snvme_change_ctrl_state(&ctrl->ctrl, NVME_CTRL_LIVE);
	if (!changed) {
		/*
...
static void nvme_rdma_error_recovery_work(struct work_struct *work)
{
	...
	if (!snvme_change_ctrl_state(&ctrl->ctrl, NVME_CTRL_CONNECTING)) {
		/* state change failure is ok if we started ctrl delete */
		enum nvme_ctrl_state state = nvme_ctrl_state(&ctrl->ctrl);
...
{
	if (!snvme_change_ctrl_state(&ctrl->ctrl, NVME_CTRL_RESETTING))
		return;

	dev_warn(ctrl->ctrl.device, "starting error recovery\n");
	queue_work(s_nvme_reset_wq, &ctrl->err_work);
}
...
static void nvme_rdma_recv_done(struct ib_cq *cq, struct ib_wc *wc)
{
	...
	if (unlikely(nvme_is_aen_req(nvme_rdma_queue_idx(queue),
				     cqe->command_id)))
		snvme_complete_async_event(&queue->ctrl->ctrl, cqe->status,
				&cqe->result);
	else
		...
		const struct blk_mq_queue_data *bd)
{
	...
	if (!nvme_check_ready(&queue->ctrl->ctrl, rq, queue_ready))
		return snvme_fail_nonready_command(&queue->ctrl->ctrl, rq);
...
static void nvme_rdma_shutdown_ctrl(struct nvme_rdma_ctrl *ctrl, bool shutdown)
{
	...
	snvme_quiesce_admin_queue(&ctrl->ctrl);
	snvme_disable_ctrl(&ctrl->ctrl, shutdown);
		...
		struct nvmf_ctrl_options *opts)
{
	...
	ret = snvme_init_ctrl(&ctrl->ctrl, dev, &nvme_rdma_ctrl_ops,
				0 /* no quirks, we're perfect! */);
	if (ret)
		goto out_kfree_queues;

	changed = snvme_change_ctrl_state(&ctrl->ctrl, NVME_CTRL_CONNECTING);
...
static void nvme_rdma_remove_one(struct ib_device *ib_device, void *client_data)
{
	...
	flush_workqueue(s_nvme_delete_wq);
}

...
The `CMakePresets.json` generated by `prepare_env.sh` records the dependency provider selected on this machine. When vcpkg fallback is needed or `--force-vcpkg` is passed, the generated `default` / `debug` / `release` presets embed its absolute toolchain path; when the automatic system check succeeds, those presets use system packages directly. Run from the project root:
...
Available presets: `default` (RelWithDebInfo) / `debug` / `release` / `system`. The first three use the dependency provider selected by the setup script; `system` never sets a vcpkg toolchain and is useful for explicitly checking system packages. List them all with `cmake --list-presets`.
...
```bash
cmake --preset default --fresh -DSNVME_KERNEL_VERSION=5.15.0-public
```

...
## Step 9 — Tutti local-NVMe contract tests

These three tests exercise the CUDA userspace path after `tutti_daemon` has
created and mounted the `ssnvme` devices. They are hardware tests, so configure
with `-DTUTTI_BUILD_HARDWARE_TESTS=ON` and run them only while the daemon and
the required mounts are live:
...
The binaries retain their historical defaults (`/dev/ssnvme0-3`, the old test
BDF table, 4 KiB namespace blocks, and a 16 KiB BAR0 value) so existing test
commands keep the same behavior. Those values are only fallback fixtures; they
must be overridden when they do not describe the running host. The common
option format is:
...
The first four fields are compatible with the `layerwise_kv_overlap` example.
The optional fields provide the hardware metadata that the contract tests used
to hard-code: namespace LBA size in bytes (power of two, at least 512), BAR0
mapping size, and namespace ID. For example, a host whose first namespace uses
512-byte LBAs and whose second uses 4 KiB LBAs can be described as:

```bash
NVME0=/dev/ssnvme0,0000:ca:00.0,/dev/snvme0n1,/mnt/nvme0,512,8192,1
NVME1=/dev/ssnvme1,0000:e3:00.0,/dev/snvme1n1,/mnt/nvme1,4096,8192,1
```

Use the BDFs, device nodes, mount paths, and BAR0 values reported by the local
daemon/driver on the machine under test. Do not infer a GPU-to-device mapping
from the numeric suffix: daemon `allowed_gpus` ACLs may allow one GPU to use
multiple NVMe devices.

### LocalNvmeDataPath contract

```bash
sudo build/cuda/bin/tutti_local_nvme_datapath_contract_test \
  --gpu 0 --device-index 0 --nvme "$NVME0"
```

Repeat `--nvme` to replace the built-in device table (up to four entries). The
single-device sections use `--device-index`; without it, the default table uses
the selected GPU index, while an overridden table uses its first entry. The
multi-device portion uses the configured entries in order and skips when the
required mounts are unavailable.

### StorageRuntime local-NVMe contract

This test accepts exactly one `--nvme` entry because all operations use one
LocalNvmeDataPath fixture:

```bash
sudo build/cuda/bin/tutti_storage_runtime_local_nvme_contract_test \
  --gpu 0 --nvme "$NVME0"
```

The request payloads remain 4 KiB as part of the original test contract; the
`block_size` field controls only Resolver/DataPath LBA conversion. Thus a
4 KiB request on a 512-byte namespace is submitted as eight NVMe blocks.

### Striped local-NVMe contract

```bash
sudo build/cuda/bin/tutti_striped_local_nvme_contract_test \
  --devices 2 --gpu 0 --nvme "$NVME0" --nvme "$NVME1"
```

`--devices 2|4` keeps its original meaning and selects the number of entries to
use. Without it, the test chooses four configured/available entries when
possible, otherwise two. StripedDataPath requires every selected namespace to
have the same logical `block_size`; the daemon performs the corresponding
topology/metadata validation, and the test should be run only with a matching
set. A missing device or CUDA runtime returns CTest's skip code `77`.

To run the three tests through CTest after the daemon is ready:

```bash
ctest --test-dir build/cuda -R \
  'tutti_(local_nvme_datapath|storage_runtime_local_nvme|striped_local_nvme)_contract_test' \
  --output-on-failure
```

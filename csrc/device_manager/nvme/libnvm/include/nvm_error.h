...
#define NVM_ERROR(x, msg) do { \
    if (x) { \
        throw error(string(msg) + strerror(errno)); \
    } \
} while (0)

#define nvm_throw_error(msg, status) \
        throw error(string(msg) + string(" ") + nvm_strerror(status));

...
static int s_nvme_setup_io_queues(struct nvme_dev *dev)
{
	...
	/*
	 * snvme B3: record the controller-granted IOQ ceiling.  This is the
	 * authoritative bound for legal QID values used by NVM_ADD_USER_QUEUE;
	 * the user QID pool will be [online_queues..ctrl_max_io_queues].
	 * Captured BEFORE any downstream code mutates nr_io_queues.
	 */
	if (result == 0)
		dev->ctrl_max_io_queues = nr_io_queues;

	/*
	 * snvme B3 cap-only path: shrink the kernel-side consumption to
	 * cap_kernel_ioq AFTER the controller negotiation so QIDs
	 * (cap_kernel_ioq+1 .. ctrl_max_io_queues] remain unused by the
	 * kernel and NVM_ADD_USER_QUEUE can claim them.  Must run before
	 * the IRQ vectors are sized for the queue count.
	 */
	if (dev->cap_kernel_ioq &&

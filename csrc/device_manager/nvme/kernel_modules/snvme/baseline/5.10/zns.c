...
static int nvme_set_max_append(struct nvme_ctrl *ctrl)
{
	...
	status = snvme_submit_sync_cmd(ctrl->admin_q, &c, id, sizeof(*id));
	if (status) {
		kfree(id);
		return status;
	}
...
int nvme_update_zone_info(struct nvme_ns *ns, unsigned lbaf)
{
	...
	status = snvme_submit_sync_cmd(ns->ctrl->admin_q, &c, id, sizeof(*id));
	if (status)
		goto free_data;
			...
			unsigned int nr_zones, report_zones_cb cb, void *data)
{
	...
	while (zone_idx < nr_zones && sector < get_capacity(ns->disk)) {
		...
		ret = snvme_submit_sync_cmd(ns->queue, &c, report, buflen);
		if (ret) {
			if (ret > 0)
				ret = -EIO;
			goto out_free;

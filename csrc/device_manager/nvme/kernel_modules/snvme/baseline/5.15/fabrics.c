...
int nvmf_reg_read32(struct nvme_ctrl *ctrl, u32 off, u32 *val)
{
	...
	ret = __snvme_submit_sync_cmd(ctrl->fabrics_q, &cmd, &res, NULL, 0, 0,
			NVME_QID_ANY, 0, 0);
...
int nvmf_reg_write32(struct nvme_ctrl *ctrl, u32 off, u32 val)
{
	...
	ret = __snvme_submit_sync_cmd(ctrl->fabrics_q, &cmd, NULL, NULL, 0, 0,
			NVME_QID_ANY, 0, 0);
	if (unlikely(ret))
		dev_err(ctrl->device,
...
int nvmf_connect_admin_queue(struct nvme_ctrl *ctrl)
{
	...
	ret = __snvme_submit_sync_cmd(ctrl->fabrics_q, &cmd, &res,
...
int nvmf_connect_io_queue(struct nvme_ctrl *ctrl, u16 qid)
{
	...
	ret = __snvme_submit_sync_cmd(ctrl->connect_q, &cmd, &res,
...
static int __init nvmf_init(void)
{
	...
	nvmf_class = class_create(THIS_MODULE, "snvme-fabrics");

			    ...
			    void *data, size_t data_len, bool auth_send)
{
	...
	ret = __snvme_submit_sync_cmd(q, &cmd, NULL, data, data_len,
...
EXPORT_SYMBOL_GPL(snvme_auth_negotiate);
...
EXPORT_SYMBOL_GPL(snvme_auth_wait);
...
static void nvme_ctrl_auth_work(struct work_struct *work)
{
	...
	ret = snvme_auth_negotiate(ctrl, 0);
	if (ret) {
		dev_warn(ctrl->device,
			 "qid 0: error %d setting up authentication\n", ret);
		return;
	}
	ret = snvme_auth_wait(ctrl, 0);
	if (ret) {
		dev_warn(ctrl->device,
	...
	for (q = 1; q < ctrl->queue_count; q++) {
		ret = snvme_auth_negotiate(ctrl, q);
		if (ret) {
			dev_warn(ctrl->device,
...
EXPORT_SYMBOL_GPL(snvme_auth_init_ctrl);

void snvme_auth_stop(struct nvme_ctrl *ctrl)
{
	cancel_work_sync(&ctrl->dhchap_auth_work);
}
EXPORT_SYMBOL_GPL(snvme_auth_stop);
...
EXPORT_SYMBOL_GPL(snvme_auth_free);

int __init nvme_init_auth(void)
{
	nvme_auth_wq = alloc_workqueue("snvme-auth-wq",
			       WQ_UNBOUND | WQ_MEM_RECLAIM | WQ_SYSFS, 0);
	if (!nvme_auth_wq)
		return -ENOMEM;

	nvme_chap_buf_cache = kmem_cache_create("snvme-chap-buf-cache",

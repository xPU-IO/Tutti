...
static void nvme_tcp_error_recovery(struct nvme_ctrl *ctrl)
{
	...
	queue_work(s_nvme_reset_wq, &to_tcp_ctrl(ctrl)->err_work);
}
...
static void nvme_tcp_fail_request(struct nvme_tcp_request *req)
{
	if (nvme_tcp_async_req(req)) {
		...
		snvme_complete_async_event(&req->queue->ctrl->ctrl,
...
static int nvme_tcp_alloc_io_queues(struct nvme_ctrl *ctrl)
{
	...
	ret = snvme_set_queue_count(ctrl, &nr_io_queues);
	if (ret)
		return ret;
...
static int nvme_tcp_configure_io_queues(struct nvme_ctrl *ctrl, bool new)
{
	...
	if (new) {
		ret = snvme_alloc_io_tag_set(ctrl, &to_tcp_ctrl(ctrl)->tag_set,
				&nvme_tcp_mq_ops,
	...
	if (!new) {
		...
		if (!snvme_wait_freeze_timeout(ctrl, NVME_IO_TIMEOUT)) {
			/*
...
static void nvme_tcp_destroy_admin_queue(struct nvme_ctrl *ctrl, bool remove)
{
	...
	if (remove)
		snvme_remove_admin_tag_set(ctrl);
...
static int nvme_tcp_configure_admin_queue(struct nvme_ctrl *ctrl, bool new)
{
	...
	if (new) {
		error = snvme_alloc_admin_tag_set(ctrl,
	...
	error = snvme_enable_ctrl(ctrl);
	if (error)
		goto out_stop_queue;

	snvme_unquiesce_admin_queue(ctrl);

	error = snvme_init_ctrl_finish(ctrl, false);
	if (error)
...
out_quiesce_queue:
	snvme_quiesce_admin_queue(ctrl);
...
static void nvme_tcp_reconnect_or_remove(struct nvme_ctrl *ctrl)
{
	...
	if (nvmf_should_reconnect(ctrl)) {
		...
		queue_delayed_work(s_nvme_wq, &to_tcp_ctrl(ctrl)->connect_work,
...
static void nvme_tcp_error_recovery_work(struct work_struct *work)
{
	...
	if (!snvme_change_ctrl_state(ctrl, NVME_CTRL_CONNECTING)) {
		/* state change failure is ok if we started ctrl delete */
		enum nvme_ctrl_state state = nvme_ctrl_state(ctrl);
...
static void nvme_tcp_teardown_ctrl(struct nvme_ctrl *ctrl, bool shutdown)
{
	...
	snvme_quiesce_admin_queue(ctrl);
	snvme_disable_ctrl(ctrl, shutdown);
		...
		struct nvmf_ctrl_options *opts)
{
	...
	ret = snvme_init_ctrl(&ctrl->ctrl, dev, &nvme_tcp_ctrl_ops, 0);
	if (ret)

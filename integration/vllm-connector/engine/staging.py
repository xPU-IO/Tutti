...
class RingWindow:
    ...
    - acquire(n)：领取下一波次的 n 个连续（环绕时分段）槽，返回
      (wave, slots)。n 须满足 1 ≤ n ≤ num_slots // 2，否则 ValueError。
    ...
    def __init__(self, buffer, num_slots: int, segment_bytes: int,
        ...
        if (not _is_int(capacity_per_wave) or capacity_per_wave <= 0
                or capacity_per_wave > num_slots // 2):
            raise ValueError(
                "capacity_per_wave 须为正整数且不超过 num_slots//2，"
    ...
    def acquire(self, n: int) -> tuple[int, list[int]]:
        ...
        if self._legacy_halves:
            ...
            slots = tuple(range(base, base + n))
        else:
            slots = tuple((self._cursor + i) % self.num_slots for i in range(n))
